      ...
      const maxOfTwo = require('../scripts/mochajsintro').maxTwo;
      ...
      describe('JS TEST WITH MOCHA AND CHAI', function(){
        ...
        it('Assertion Failed. [5] is not the list containing 3s from the list [1,2,3,3]', function(){
          assert.equal(elementOf3()),areEqualArrays([5]);
        });   

        it('Assertion Pass. product of [1, 2, 3, 4] is 135.', function(){
          assert.equal(arrayProduct( ),135);
        });
        
       // =============================MAP and FILTER=========================
        it('Assertion Pass. product of [1, 2, 3] by 10 [10,20,30].', function(){
          assert.equal(myMap([1,2,3],s=>s*10 ),[10,20,30]);
        });
        it('Assertion Pass. product of [1, 2, 3] by 10 [10,20,30].', function(){
          assert.equal(myMap([1,2,3],s=>s*5 ),[5,10,15]);
        });
        it('Assertion Pass. product of [1, 2, 3] by 10 [10,20,30].', function(){
          assert.equal(myMap([1,2,3],s=>s*1 ),[1,2,3]);
        });
  

        it('Assertion Pass. Implementation of my filter numbers under 10 & above 5 [6,7,8,9].', function(){
          assert.equal(myFilter([1,2,3,4,5,6,7,8,9],s=>s>5 ),[6,7,8,9]);
        });      it('Assertion Pass. Implementation of my filter numbers under 10 & above 5 [6,7,8,9].', function(){
          assert.equal(myFilter([1,2,3,4,5,6,7,8,9],s=>s>8 ),[9]);
        });      it('Assertion Pass. Implementation of my filter numbers under 10 & above 5 [6,7,8,9].', function(){
          assert.equal(myFilter([1,2,3,4,5,6,7,8,9],s=>s>1 ),[2,3,4,5,6,7,8,9]);
        });
      });
